...
//    unsigned int cam_mode;
    ...
    // Move stuff to variables on GPU
    ...
    // New pointers to hold the final scaled motion field.
    float *dx_scaled_dev, *dy_scaled_dev, *dz_scaled_dev;
...
//								 float *g_mask, unsigned int levels_host, unsigned int cam_mode_host, float fovh_host, float fovv_host);
    ...
    __host__ void applyPositionalChangeUsingCuBLAS(float tx, float ty, float tz, int num_points);
    ...
    /*
     * Scale the motion field by dividing each element by dt.
     * Instead of performing the scaling operation in place,
     * this function allocates (if necessary) new device buffers 
     * and copies + scales the motion field.
     *
     * @param dt: The time interval (in seconds).
     */
    __host__ void scaleMotionField(const float dt);
};
...
// Bridge function prototype allowing non–CUDA code to call the positional change.
