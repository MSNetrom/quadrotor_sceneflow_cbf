...
bool PD_flow_opencv::loadRGBDFrames()
{
	...
	// Log max and min of intensity1
	double minIntensity, maxIntensity;
	cv::minMaxLoc(intensity1, &minIntensity, &maxIntensity);
	...
	// Log max and min of depth1
	printf("Depth 0,0: %d\n", static_cast<int>(depth1.at<unsigned short>(0,0)));
	double minDepth, maxDepth;
	cv::minMaxLoc(depth1, &minDepth, &maxDepth);
	printf("Depth min: %f, max: %f\n", minDepth, maxDepth);

	depth1.convertTo(depth_float, CV_32FC1, 1.0 / 1000.0);
	for (unsigned int v=0; v<height; v++)
		for (unsigned int u=0; u<width; u++)
	...
	// Log max and min of intensity2
	double minIntensity2, maxIntensity2;
	cv::minMaxLoc(intensity2, &minIntensity2, &maxIntensity2);
	printf("Intensity (frame2) min: %f, max: %f\n", minIntensity2, maxIntensity2);
	...
	printf("Depth 0,0 (frame2): %d\n", static_cast<int>(depth2.at<unsigned short>(0,0)));
	double minDepth2, maxDepth2;
	cv::minMaxLoc(depth2, &minDepth2, &maxDepth2);
	printf("Depth (frame2) min: %f, max: %f\n", minDepth2, maxDepth2);

	depth2.convertTo(depth_float, CV_32FC1, 1.0 / 1000.0);
	for (unsigned int v=0; v<height; v++)
		for (unsigned int u=0; u<width; u++)
			Z[v + u*height] = depth_float.at<float>(v,u);
