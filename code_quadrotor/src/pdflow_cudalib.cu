...
__host__ void CSF_cuda::copyR_gpu() {
    cudaMemcpy(r_x_gpu, xx_dev[2], rows * cols * sizeof(float), cudaMemcpyDeviceToDevice);
    cudaMemcpy(r_y_gpu, yy_dev[2], rows * cols * sizeof(float), cudaMemcpyDeviceToDevice);
    cudaMemcpy(r_z_gpu, depth_dev[2], rows * cols * sizeof(float), cudaMemcpyDeviceToDevice);
}
...
//    const unsigned int height = 480/cam_mode;
    ...
    // Print all level data
    std::cout << "pyr_levels: " << pyr_levels << std::endl;
    ...
    // Now, also allocate memory for the scaled motion field.
    // Depending on your application, you might use 'width * height' or the
    // actual number of elements for the computed motion field (e.g., rows_i * cols_i).
    err = cudaMalloc((void**)&dx_scaled_dev, rows * cols * sizeof(float));
    if (err != cudaSuccess)
        fprintf(stderr, "cudaMalloc failed for dx_scaled_dev: %s\n", cudaGetErrorString(err));

    err = cudaMalloc((void**)&dy_scaled_dev, rows * cols * sizeof(float));
    if (err != cudaSuccess)
        fprintf(stderr, "cudaMalloc failed for dy_scaled_dev: %s\n", cudaGetErrorString(err));
        
    err = cudaMalloc((void**)&dz_scaled_dev, rows * cols * sizeof(float));
    if (err != cudaSuccess)
        fprintf(stderr, "cudaMalloc failed for dz_scaled_dev: %s\n", cudaGetErrorString(err));

    cudaMalloc((void**)&r_x_gpu, rows * cols * sizeof(float));
    cudaMalloc((void**)&r_y_gpu, rows * cols * sizeof(float));
    cudaMalloc((void**)&r_z_gpu, rows * cols * sizeof(float));

    cudaMalloc((void**)&r_dot_x_gpu, rows * cols * sizeof(float));
    cudaMalloc((void**)&r_dot_y_gpu, rows * cols * sizeof(float));
    cudaMalloc((void**)&r_dot_z_gpu, rows * cols * sizeof(float));
}
...
__host__ void CSF_cuda::readParameters(unsigned int width_host, unsigned int height_host, float fx_host, float fy_host, float cx_host, float cy_host,
...
//    const unsigned int width = 640/cam_mode;
    ...
    cudaFree(dx_dev); cudaFree(dy_dev); cudaFree(dz_dev);
    cudaFree(r_x_gpu); cudaFree(r_y_gpu); cudaFree(r_z_gpu);
    cudaFree(r_dot_x_gpu); cudaFree(r_dot_y_gpu); cudaFree(r_dot_z_gpu);
}
...
//    const float inv_f_i = 2.f*tan(0.5f*fovh)/float(cols_i);
    const float inv_fx_i = float(width)/(float(cols_i)*fx);
    const float inv_fy_i = float(height)/(float(rows_i)*fy);

//    const float disp_u_i = 0.5f*(cols_i-1);
//    const float disp_v_i = 0.5f*(rows_i-1);
    const float cx_i = cx*float(cols_i)/float(width);
    const float cy_i = cy*float(rows_i)/float(height);

    xx_dev[level][index] = (u - cx_i)*depth_dev[level][index]*inv_fx_i;
    yy_dev[level][index] = (v - cy_i)*depth_dev[level][index]*inv_fy_i;
}
...
//	const float inv_f = 2.f*tanf(0.5f*fovh)/float(cols);
    const float inv_fx = float(width)/(float(cols)*fx);
    const float inv_fy = float(height)/(float(rows)*fy);
...
void GaussianPyramidBridge(CSF_cuda *csf, unsigned int levels, unsigned int width, unsigned int height)
{
    for (unsigned int i=0; i<levels; i++)
    {
        const unsigned int cols_i_aux = width/(powf(2,i));
        const unsigned int rows_i_aux = height/(powf(2,i));
...
__host__ void CSF_cuda::scaleMotionField(const float dt)
{
    // We assume that dx_dev, dy_dev, dz_dev have been computed on the GPU.
    // The size of the motion field is determined as follows.
    const int N = rows_i * cols_i;
    ...
    // First, copy the motion field computed (unscaled) from dx_dev -> dx_scaled_dev, etc.
    status = cublasScopy(handle, N, dx_dev, 1, r_dot_x_gpu, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "cublasScopy failed for dx_dev\n");
    }
    status = cublasScopy(handle, N, dy_dev, 1, r_dot_y_gpu, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "cublasScopy failed for dy_dev\n");
    }
    status = cublasScopy(handle, N, dz_dev, 1, r_dot_z_gpu, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "cublasScopy failed for dz_dev\n");
    }

    // Compute the scaling factor.
    const float scale = 1.0f / dt;

    // Scale the copied arrays in place. Each element will be multiplied by scale.
    status = cublasSscal(handle, N, &scale, r_dot_x_gpu, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "cublasSscal failed for r_dot_x_gpu\n");
    }
    status = cublasSscal(handle, N, &scale, r_dot_y_gpu, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "cublasSscal failed for r_dot_y_gpu\n");
    }
    status = cublasSscal(handle, N, &scale, r_dot_z_gpu, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "cublasSscal failed for r_dot_z_gpu\n");
    }
...
// Helper kernel to fill an array with ones.
__global__ void fillOnesKernel(float* ones, int num_points) {
...
__host__ void CSF_cuda::applyPositionalChangeUsingCuBLAS(float tx, float ty, float tz, int num_points) {
    // Choose a pyramid level to work on.
    int level = level_image;
    ...
    // Allocate a device vector "d_ones" of length num_points and fill it with ones.
    float* d_ones;
    cudaError_t err = cudaMalloc((void**)&d_ones, num_points * sizeof(float));
    if (err != cudaSuccess) {
        fprintf(stderr, "cudaMalloc failed for d_ones: %s\n", cudaGetErrorString(err));
        cublasDestroy(handle);
        return;
    }
    ...
    fillOnesKernel<<<gridSize, blockSize>>>(d_ones, num_points);
    cudaDeviceSynchronize();
    
    // For each coordinate, perform: coordinate = coordinate + t_coordinate * d_ones
    status = cublasSaxpy(handle, num_points, &tx, d_ones, 1, xx_dev[level], 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "cublasSaxpy failed for x coordinate\n");
    }
    
    status = cublasSaxpy(handle, num_points, &ty, d_ones, 1, yy_dev[level], 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "cublasSaxpy failed for y coordinate\n");
    }
    
    status = cublasSaxpy(handle, num_points, &tz, d_ones, 1, depth_dev[level], 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "cublasSaxpy failed for z coordinate (depth)\n");
    }
...
// Bridge function so that non–CUDA code can call the above:
void ApplyPositionChangeBridge(CSF_cuda *csf, float tx, float ty, float tz) {
    int num_points = csf->rows_i * csf->cols_i; // or another chosen size
    csf->applyPositionalChangeUsingCuBLAS(tx, ty, tz, num_points);
}
